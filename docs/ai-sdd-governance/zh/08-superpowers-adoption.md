...
English version: [../08-superpowers-adoption.md](../08-superpowers-adoption.md)

## 定位

Superpowers 是内部团队默认 AI-SDD 执行工作流。

它本身不是完整企业交付方法论。项目仍依赖 SDD specs、quality gates、owner review、CI/CD、安全扫描和验收管理。
...
- 先按 spec review，再做 code quality review。
- 验证后才声明完成。
...
- 生产风险变更。
- 架构决策。
- 需要 Module Owner approval 的变更。

必需流程：

1. 使用完整 SDD Story Spec 和 Technical Spec。
...
- 评审计划、执行 workflow tiering，需要时请求 code review。

QA：

- 评审 Test Spec、验收证据和验证结果。

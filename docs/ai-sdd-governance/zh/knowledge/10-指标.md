...
英文版：[../../knowledge/10-metrics.md](../../knowledge/10-metrics.md)
...
- 定义：从 SDD Spec 批准到 MR ready for 评审 的时间。
- 用途：衡量 AI 辅助开发速度。
...
- 复盘供应商交付质量、模块 负责人hip、安全态势和推广进展。

## 要点回顾

- 指标驱动改进，不是个人考核——按趋势看，不按分数看。
- 效率、质量、一致性指标要一起看；AI Code Adoption Rate 单独看具有误导性，必须并列 Defect Escape Rate。
- 每周/每两周/每月节奏分别对应交付健康、流程调优、供应商与推广复盘。
- 指标回路靠"反馈回规格、prompts、harness 政策、测试"闭合——没有反馈的度量是负担。

## 下一篇

- [总结：从概念到交付](11-概念到交付.md)——用一个 Story 走完你刚读过的每一篇，包括本篇中每个指标在那个 Story 上具体怎么记录。
